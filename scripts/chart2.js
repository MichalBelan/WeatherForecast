...
function loadTemperatureChart(apiUrl, label) {
  fetch(apiUrl)
    ...
    .then((data) => {
      console.log(data);
      // Výpis údajov o čase, teplote, vlhkosti, kóde počasia a tlaku z odpovede API
      ...
      chartData.addColumn("number", "Relative Humidity (%)");
      chartData.addColumn("number", "Weather Code");
      chartData.addColumn("number", "Surface Pressure (hPa)");

      for (let i = 0; i < time.length; i++) {
        chartData.addRow([time[i], temperature[i], humidity[i], weatherCode[i], pressure[i]]);
      }
