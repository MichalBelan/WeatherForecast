document.addEventListener("DOMContentLoaded", function () {
  ...
  // Načítanie údajov o počasí pri načítaní stránky
  loadData(forecastApiUrl);

  const forecastBtn = document.getElementById("forecastBtn");
  const historicalBtn = document.getElementById("historicalBtn");
  // Načítanie predpovede alebo historických údajov o počasí na základe kliknutia na tlačidlo
  forecastBtn.addEventListener("click", () => loadData(forecastApiUrl));
  historicalBtn.addEventListener("click", () => loadData(historicalApiUrl));
});
